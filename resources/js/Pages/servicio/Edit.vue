    ...
    <div class="max-w-2xl mx-auto p-6 bg-base-100 shadow-md rounded-box">
      ...
      <form @submit.prevent="submit" class="space-y-4">
        ...
        <div>
          ...
          <input v-model="form.tipo_muestra" type="text" id="tipo_muestra" class="input input-bordered w-full" />
          <p class="text-error mt-1" v-if="form.errors.tipo_muestra">{{ form.errors.tipo_muestra }}</p>
        </div>

        <div>
          <label for="precio" class="label">Precio</label>
          <input v-model="form.precio" type="number" step="0.01" id="precio" class="input input-bordered w-full" />
